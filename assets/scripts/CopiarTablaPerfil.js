var PerfileUno = document.querySelector("#tablaPerfilesUno tbody"), //Cuerpo de la primera tabla
    PerfilDos = document.querySelector("#tablaPerfilesDos tbody"), //Cuerpo de la segunda tabla
    btnPefil = document.querySelector("#btnCopiarPerfiles"), //Botón que copiará los datos de las filas seleccionadas
    seleccion = [], //Arreglo que almacenará a las filas seleccionadas
    seleccionarP = function(event){ //Función a ejecutarse para seleccionar una fila
    ...
    copiar = function(){ //Función a ejecutarse para copiar los datos de las filas seleccionadas en la segunda tabla
        if (seleccion.length){ //Si hay filas seleccionadas
            for (var i = 0, l = seleccion.length; i < l; i++){ //Se recorre a dicho conjunto
                var tr = PerfilDos.insertRow(), //Se inserta una nueva fila en la segunda tabla
    ...
    PerfileUno.addEventListener("click", seleccionarP, false);

    //Cuando se pulse el botón, se ejecutará el siguiente conjunto de instrucciones
    btnPefil.addEventListener("click", copiar, false);